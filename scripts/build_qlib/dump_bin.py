...
#
# Local edits for this repo:
# - `logging` instead of `loguru` (no extra dependency)
# - `fname_to_code` / `code_to_fname` inlined from `qlib.utils` (package may be absent)
# - `fire` is optional in __main__
...
def code_to_fname(code: str) -> str:
    """stock code to file name (qlib.utils)."""
...
def fname_to_code(fname: str) -> str:
    """file name to stock code (qlib.utils)."""
...
class DumpDataBase:
    ...
    def save_calendars(self, calendars_data: list):
        write_calendars(self.qlib_dir, calendars_data, freq=self.freq)

    def save_instruments(
        self,
        instruments_data: Union[list, pd.DataFrame],
        *,
        file_name: str | None = None,
    ):
        self._instruments_dir.mkdir(parents=True, exist_ok=True)
        out_name = file_name if file_name is not None else self.INSTRUMENTS_FILE_NAME
        instruments_path = str(self._instruments_dir.joinpath(out_name).expanduser().resolve())
...
def write_calendars(qlib_dir: Union[str, Path], calendars_data: list, *, freq: str = "day") -> Path:
    """
    Write `calendars/{freq}.txt` using the same rules as `DumpDataBase.save_calendars`
    (`np.savetxt`, `DAILY_FORMAT` / `HIGH_FREQ_FORMAT`).
    """

    root = Path(qlib_dir).expanduser().resolve()
    cal_dir = root.joinpath(DumpDataBase.CALENDARS_DIR_NAME)
    cal_dir.mkdir(parents=True, exist_ok=True)
    calendar_format = DumpDataBase.DAILY_FORMAT if freq == "day" else DumpDataBase.HIGH_FREQ_FORMAT
    result_calendars_list = [pd.Timestamp(x).strftime(calendar_format) for x in calendars_data]
    out = cal_dir.joinpath(f"{freq}.txt")
    np.savetxt(str(out.expanduser().resolve()), result_calendars_list, fmt="%s", encoding="utf-8")
    return out
        ...
        with ProcessPoolExecutor(max_workers=self.works) as executor:
            ...
            with tqdm(total=len(futures)) as p_bar:
                for _future in as_completed(futures):
                    ...
                    p_bar.update()
            logger.info("dump bin errors: %s", error_code)
...
def make_process_bin_writer(
    output_dir: Path,
    *,
    freq: str = "day",
    max_workers: int = 16,
    exclude_fields: str = "symbol",
    include_fields: str = "",
) -> DumpDataAll:
    """
    Build a DumpDataAll instance suitable for calling `_dump_bin` from to_qlib (build_qlib pipeline).

    `data_path` points at `qlib_dir/features`, where top-level `*.csv` is empty so `df_files` is
    unused; calendars/instruments/bin output still go under `qlib_dir`.
    """

    out = Path(output_dir).expanduser().resolve()
    features_root = out / DumpDataBase.FEATURES_DIR_NAME
    features_root.mkdir(parents=True, exist_ok=True)
    return DumpDataAll(
        data_path=str(features_root),
        qlib_dir=str(out),
        backup_dir=None,
        freq=freq,
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(
            "Optional CLI dependency missing: install `fire` to run dump_bin.py as a script "
            "(e.g. `pip install fire`)."
        ) from exc
