...
from scripts.qlib.runtime.mlflow import fixed_mlflow_exp_manager
...
def init_qlib_for_backtest(args: argparse.Namespace) -> str:
    ...
    if not _inst_path.is_file():
        raise SystemExit(
            ...
            "请确认已下载对应指数权重（如 data/<日期>/index/index_weight/），并重新运行 scripts.build_qlib.to_qlib（或 make process）。"
        )
