...
from qlib_lab.handler.features.full import full_feature_config_pruned_top508
from qlib_lab.handler.label import DEFAULT_LABEL_EXPR


class QuantFoundryRawFields(DataHandlerLP):
    """Alpha158 + Alpha360（去重）+ 基本面/筹码/价量/成交额扩展；默认特征为 gain 前 508 列裁剪集。"""
    ...
    def get_feature_config(self) -> tuple[list[str], list[str]]:
        return full_feature_config_pruned_top508()
